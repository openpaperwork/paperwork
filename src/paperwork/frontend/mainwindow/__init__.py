...
from paperwork.frontend.mainwindow.pages import BasePageDrawer
...
class MainWindow(object):
    ...
    def _show_page_internal(self, page, force_refresh=False):
        ...
        if drawer is not None:
            self.img['canvas'].get_vadjustment().set_value(
                drawer.position[1] - BasePageDrawer.MARGIN
            )
    ...
    def compute_zoom_level(self, img_size, other_pages):
        if self.layout == "grid":
            ...
            canvas_width -= len(other_pages) * (2 * BasePageDrawer.MARGIN)
            if total_width > 0:
